...
nativeLibs, visualizers, sources :: FilePath
nativeLibs    = "native_libs"
visualizers   = "visualizers"
sources       = "src"

nativeLibsBin :: FilePath
...
data Hook input output m = Hook
    { _action :: input -> m output
    } deriving (Show)
...
package build pack = do
    ...
    inited <- runHook (pack ^. initialize) (InitInput build)
    ...
    built <- runHook (pack ^. buildNativeLibs) (BuildInput build inited)
